...
int al_deleteArrayList(ArrayList* pList)
{
    ...
    if(pList != NULL)
    {
        free(pList->pElements); //estoy liberando el espacio para alocar punteros
        free(pList);
    }
 ...
 */
int al_isEmpty(ArrayList* pList) //verificamos si esta vacio
{
    int returnAux = -1;

    if(pList != NULL)
    {
        if(pList->pElements == 0)
        {
            returnAux == 1;
        }
        else
 ...
 */
ArrayList* al_subList(ArrayList* pList,int from,int to) //1.hacer un nuevo arraylist.2.llamo a newarralist
{
    void* returnAux = NULL;
    int auxSubList;

    auxSubList = al_newArrayList();
